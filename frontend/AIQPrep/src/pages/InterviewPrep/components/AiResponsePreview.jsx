...
export default function AiResponsePreview({content}){
    // console.log("AiResponsePreview result",content)
                ...
                <ReactMarkdown 
                    ...
                    components={{
                        code({node,className,children, ...props}){
                            ...
                            //This extracts the programming language from the className of the markdown code block, like language-js, language-python, etc.
                            const match = /language-(\w+)/.exec(className || '');
                            const language = match ? match[1] : '';

                            // If there's no className (and hence no language), it's likely inline code.
                            const isInline = !className;

                            // For code blocks: It uses a custom <CodeBlock /> component to properly format and highlight the code.
                            // For inline code: It returns a styled <code> tag directly.
                            return (
                                !isInline ? 
                                (
                                    <CodeBlock
    ...
    return (
        <div className="relative my-6 rounded-lg overflow-hidden bg-gray-50 border border-gray-200">
            <div className="flex justify-between items-center px-4 py-2 bg-gray-100 border-b border-gray-200">
                ...
                <button onClick={copyCode} aria-label="Copy Code" className="text-gray-500 hover:text-gray-700 focus:outline-none relative group cursor-pointer"> 
                    {
                        copied ?
